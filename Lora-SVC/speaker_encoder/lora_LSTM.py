...
from speaker.utils.audio import AudioProcessor as AudioProcessor_ori
...
class AudioProcessor(object):
        ...
        if self.signal_norm:
            ...
            if self.symmetric_norm:
                ...
                if self.clip_norm:
                    ...
                    # )
                    S_norm = torch.clamp(
                        S_norm, min=-self.max_norm, max=self.max_norm  # pylint: disable=invalid-unary-operand-type
                    )
                return S_norm
            else:
                ...
                if self.clip_norm:
                    ...
                    S_norm = np.clamp(S_norm, min=0, max=self.max_norm)
                return S_norm
        else:
            return S
    ...
    def _amp_to_db(self, x: np.ndarray) -> np.ndarray:
        ...
        if self.base == 10:
            return self.spec_gain * torch.log10(torch.clamp(input=x, min=1e-5))
        else:
            return self.spec_gain * torch.log(torch.clamp(input=x, min=1e-5))
    ...
    def apply_preemphasis(self, x: np.ndarray) -> np.ndarray:
        ...
        return torchaudio.functional.lfilter(x, torch.tensor([1, 0], dtype=torch.float32), 
        torch.tensor([1, -self.preemphasis], dtype=torch.float32), clamp=False)
    ...
    def _linear_to_mel(self, spectrogram: np.ndarray) -> np.ndarray:
        ...
        """
        mel_basis = torch.from_numpy(self.mel_basis).to(spectrogram.device)
    ...
    def melspectrogram(self, y: np.ndarray) -> np.ndarray:
        ...
        if self.do_amp_to_db_mel:
            ...
            S = self._amp_to_db(self._linear_to_mel(torch.abs(D)))
        else:
            # S = self._linear_to_mel(np.abs(D))
            S = self._linear_to_mel(torch.abs(D))
        # return self.normalize(S).astype(np.float32)
        return self.normalize(S).float()
    ...
    def trim_silence(self, wav):
        ...
        idx = librosa.effects.trim(wav.detach().cpu().numpy(), top_db=self.trim_db, frame_length=self.win_length, hop_length=self.hop_length)[
            1
        ]
        return wav[idx[0]:idx[1]]
    ...
    def load_wav(self, filename: str, sr: int = None) -> np.ndarray:
        ...
        print(self.do_trim_silence, self.do_sound_norm)
...
class LoraLSTM(nn.Module):
    ...
    def __init__(self, config_path='speaker_pretrain/config.json', model_path='speaker_pretrain/best_model.pth.tar', use_cuda=True):
        super().__init__()
        ...
        self.speaker_encoder_ap = AudioProcessor(**config.audio)
        # # normalize the input audio level and trim silences
        # self.speaker_encoder_ap.do_sound_norm = True
        # self.speaker_encoder_ap.do_trim_silence = True
        self.speaker_encoder_ap.do_sound_norm = False
        self.speaker_encoder_ap.do_trim_silence = False
        ...
        self.speaker_encoder.load_checkpoint(model_path, eval=False, use_cuda=use_cuda)
        self.use_cuda = use_cuda
        # if self.use_cuda:
        #     self.speaker_encoder_ap.to('cuda')


    def embedding_from_file(self, wav_file):
        ...
        embed = self.speaker_encoder.compute_embedding(spec)
        embed = embed.squeeze()
        return embed
    ...
    def embedding(self, waveform):
        ...
        spec = self.speaker_encoder_ap.melspectrogram(waveform.cpu())
        spec = spec.T
        if self.use_cuda:
            spec = spec.cuda()
        spec = spec.unsqueeze(0)
    ...
    def forward(self, x, enroll_embs=None, return_emb=False):
