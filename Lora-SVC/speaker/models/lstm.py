...
class LSTMSpeakerEncoder(nn.Module):
    ...
    def compute_embedding(self, x, num_frames=250, num_eval=10, return_mean=True):
        ...
        frames_batch = torch.cat(frames_batch, dim=0) # (bs * fb, #frame, d)
        embeddings = self.inference(frames_batch) # (bs * fb, D)

        final_embeddings = []
        if return_mean:
            for b_idx in range(x.shape[0]):
                # n_sub_per_sample = frames_batch.shape[0] // x.shape[0]
                # my_idx = list(range(b_idx*n_sub_per_sample, (b_idx+1)*n_sub_per_sample, n_sub_per_sample))
                my_idx = list(range(b_idx, frames_batch.shape[0], x.shape[0]))
                final_embeddings += [embeddings[my_idx, :].mean(0).unsqueeze(0)]
